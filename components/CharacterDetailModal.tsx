...
export const CharacterDetailModal: React.FC<CharacterDetailModalProps> = ({ character, gameState, onClose, onCustomize, lang, manifest, images, clubs }) => {
  ...
  const renderDetailTab = () => {
    ...
    <>
        ...
        <View style={styles.detailList}><Text style={styles.detailText}><Text style={styles.bold}>Học vấn:</Text> {getEducationDisplay(character, lang)}</Text><Text style={[styles.detailText, { color: '#22c55e' }]}><Text style={styles.bold}>Sự nghiệp:</Text> {character.careerTrack ? t(character.careerTrack, lang) : 'Chưa có'}</Text></View>
